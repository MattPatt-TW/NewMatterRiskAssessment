...
def btn_Duplicate_MRA_Template(s, event):
  ...
  for x in dg_MRA_Templates.Items:
    tCount += 1
    if str(x['Name']) == str(tempName):
      dg_MRA_Templates.SelectedIndex = tCount
      dg_MRA_Templates.ScrollIntoView(dg_MRA_Templates.Items[tCount])
      # TODO: scroll into view code added...
      #       Prior tests revealed that this never got triggered - so have changed how we address 'x' to see if this makes it work
      #       FFS... I've only just clocked that our very next line of code refreshes the datagrid! So of course our current selection gets wiped!!
...
def Edit_MRA_Template(s, event):
  ...
  if tmpEditingTypeID != -1:
    #! this is the TypeID for the 'editing' version of the template, so we will load this one
    TypeIDtoUse = origItem['EditingTypeID']
    #MessageBox.Show("There's already an 'EditingTypeID': {0}".format(TypeIDtoUse), "DEBUG - Edit MRA Template")
  else:
    ...
    if nameContainsEditing == False:
      ...
      # Insert main 'header' row items for the new template
      insertSQL = """INSERT INTO Usr_MRA_TemplateTypes (TypeName, Is_MRA, ValidityPeriodDays, TypeID, Hidden, EffectiveFrom, EffectiveTo, InternalNote, UsersNote) 
                    SELECT CONCAT(TypeName, ' [Editing]'), 'Y', ValidityPeriodDays, {newTypeID}, 'Y', NULL, '2100-01-01 00:00:00.000', '', '' 
                    FROM Usr_MRA_TemplateTypes WHERE TypeID = {idTOcody}""".format(newTypeID=nextTypeID, idTOcody=idItemToCopy)
      
      #MessageBox.Show("No 'EditingTypeID' set, so copying selected template as new to edit.\nOrigItemID: {0}\nNew TypeID: {1}\nNext QID: {2}\nNext AnsID: {3}\n\nInsert SQL:\n{4}".format(idItemToCopy, nextTypeID, nextQID, nextAnsID, insertSQL), "DEBUG - Edit MRA Template")
      ...
      if didAddQs == 'Error':
        addedQs = False
        MessageBox.Show("An error occurred copying the Questions, using SQL:\n{0}".format(copyQ_SQL), "Error: Duplicating Matter Risk Assessment...")
        return
      else:
        addedQs = True
      ...
      # COPYING ANSWERS OVER
      #! 30/07/2025 - New version that doesn't need loop process
      #! 17/09/2025 - Updated 'myQuestionList' to ensure 'RowNumQ' reflects QuestionGroup ordering and then DisplayOrder
      copyAnsSQL = """WITH myAnswerList AS (
                        SELECT 'GrpName' = TA.GroupName, 'AnsText' = TA.AnswerText, 'Score' = TA.Score, 
                              'EmailComment' = TA.EmailComment, 
                              'RowNum' = ROW_NUMBER() OVER (ORDER BY TQ.DisplayOrder, TA.DisplayOrder, TA.AnswerText), 
      ...
      if didAddAs == 'Error':
        addedAs = False
        MessageBox.Show("An error occurred copying the Answers, using SQL:\n{0}".format(copyAnsSQL), "Error: Duplicating Matter Risk Assessment...")
        return
      else:
        addedAs = True
      ...
      #MessageBox.Show("updateSQL: " + str(updateSQL), "Duplicate Matter Risk Assessment - Answers for Questions...")
      #_tikitResolver.Resolve("[SQL: UPDATE Usr_MRA_TemplateQs SET SourceID = NULL WHERE SourceID IS NOT NULL]")
      #! temp disabling this as if there's any error with copying, we can use this to retroactively update Answers based on these
  ...
  if TypeIDtoUse is not None and TypeIDtoUse != '':
    #lbl_EditRiskAssessment_Name.Content = "{0} [Editing]".format(origItem['Name'])
    #lbl_EditRiskAssessment_ID.Content = TypeIDtoUse   #origItem['Code']
    ...
    if nameContainsEditing == True:
      ...
      tb_CopyOfMRAid.Text = str(origItem['Code'])
      tb_CopyOfMRAname.Text = str(origItem['Name'])
  else:
...
def refresh_MRA_Questions(s, event):
  ...
  getTableSQL = """SELECT MRATQ.DisplayOrder, 
                   ...
                   ORDER BY MRATQ.QGroupID, MRATQ.DisplayOrder""".format(tb_ThisMRAid.Text)    #lbl_EditRiskAssessment_ID.Content)
...
btn_Edit_MRATemplate.Click += Edit_MRA_Template
