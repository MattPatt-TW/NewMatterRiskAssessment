...
class UsersList(object):
  def __init__(self, myFEName, myBranch, myOurRef, myMRAname, myMRAexp, myOV_ID, myEntRef, myMatNo, myType, myClName, myMatDesc, myFEemail):
      ...
      while dr.Read():
        if not dr.IsDBNull(0):
          ...
          myFEitems.append(UsersList(myFEName=iFE, myBranch=iBranch, myOurRef=iOurRef, myMRAname=iMRAName, myMRAexp=iMRAExpiry, 
                                     myOV_ID=iOVID, myEntRef=iEntRef, myMatNo=iMatNo, myType=iType, myClName=iClName, 
                                     myMatDesc=iMatDesc, myFEemail=iFEemail))
...
def refresh_Preview_MRA(s, event):
  ...
  #MessageBox.Show("Generating SQL...", "Refreshing list (datagrid of questions)")
  mySQL = """SELECT '0-QGroup' = QGs.Name, 
                    '1-DisplayOrder' = MRAD.DisplayOrder, 
                    '2-QText' = TQs.QuestionText, 
                    '3-Answer' = CASE WHEN MRAD.AnswerListToUse = '(TextBox)' THEN MRAD.tbAnswerText 
                                      ELSE (SELECT AnswerText FROM Usr_MRA_TemplateAs TAs WHERE TAs.AnswerID = MRAD.SelectedAnswerID AND TAs.QuestionID = MRAD.QuestionID) END, 
                    '4-Notes' = MRAD.Notes, 
...
def Approve_Button_Clicked(s, event):
  #! 12/12/2025: After testing the other update I made to naming the 'LocalName' field to source MRA name plus a number in brackets, I found that this
  #! approve takes about 10 seconds to run, which could be due to the sheer (16?) SQL statements being executed here. 
  #! Some initial thoughts were that we can optimise by combining some directly into the 'INSERT' statement, however, I'm wondering why I'm not pulling-in
  #! the 'ClientName' and 'MatterDescription' from the Fee Earner list/datagrid, as we do have the info in there already!  

  if dg_FeeEarners.SelectedIndex == -1:
    return
  
  # get input variables that we'll put into our Task Centre trigger table
  ovID = lbl_MRA_OV_ID.Content
  selItem = dg_FeeEarners.SelectedItem
  entRef = selItem['EntityRef']
  matNo = selItem['MatterNo']
  feFullName = selItem['FEName']

  errorCount = 0
  errorMessage = ""

  tmpOurRef = selItem['OurRef']
  tmpEmailTo = selItem['FEEmail']
  #tmpEmailTo = runSQL("SELECT EMailExternal FROM Users WHERE FullName = '{0}'".format(feFullName))
  tmpEmailCC = runSQL("SELECT EMailExternal FROM Users WHERE Code = '{0}'".format(_tikitUser))
  tmpToUserName = runSQL(codeToRun="SELECT ISNULL(Forename, FullName) FROM Users WHERE FullName = '{0}'".format(feFullName), apostropheHandle=1)
  tmpMatDesc = selItem['MatDesc']
  #tmpMatDesc = runSQL(codeToRun="SELECT Description FROM Matters WHERE EntityRef = '{0}' AND Number = {1}".format(entRef, matNo), apostropheHandle=1)
  tmpClName = selItem['ClientName']
  #tmpClName = runSQL(codeToRun="SELECT LegalName FROM Entities WHERE Code = '{0}'".format(entRef), apostropheHandle=1)
  tmpAddtl1 = selItem['MRAName']
  ...
  countMatterLocks = runSQL(countMatterLocksSQL)
...
def createNewMRA_BasedOnCurrent(s, event):
  ...
  #! 12/12/2025: I didn't like this because we keep getting ' (copy of xxx)' appended to the name - so have improved this...
  #! we  now set the name without the 'copy of' text, and append a number representing the NEXT_IN_SEQUENCE for that TypeID/Matter
  #! Need to fix historic records separately as no point coding into this function now
  # 1: Get raw 'name' of Template Name
  # 2: Get 'NEXT_IN_SEQUENCE' number for this Matter/TypeID
  # 3: Oooh... we're hard-coding the 'expiry' to be 4 weeks from today - initially thought this is wrong, but this is the monthly review cycle for High Risk matters, so is actually correct

  # get input variables
  selItem = dg_FeeEarners.SelectedItem
  idItemToCopy = selItem['OV_ID']
  entRef = selItem['EntityRef']
  matNo = selItem['MatterNo']

  # get the 'TypeID' of the template we're meant to be copying
  tmpTypeID = runSQL(codeToRun="SELECT TypeID FROM Usr_MRA_TemplateTypes WHERE TypeID = (SELECT TypeID FROM Usr_MRA_Overview WHERE ID = {0})".format(idItemToCopy)) 
  # get the 'TypeName' of the template we're copying
  tmpTypeName = runSQL(codeToRun="SELECT TypeName FROM Usr_MRA_TemplateTypes WHERE TypeID = (SELECT TypeID FROM Usr_MRA_Overview WHERE ID = {0})".format(idItemToCopy)) 
  #! ^ note: the above assumes we should be using the SAME TypeID as the one we're copying from, however, see other notes below about possibly amending
  #!         this in future to pick-up the 'latest' TypeID for the MRA. As Question and Answers could be alrtered, we need to also consider how we handle
  #!         the 'Score' that was recorded against each Question - do we just copy that across, or do we need to re-calculate based on latest template?

  # get the count of items with that particular TypeID on the current matter
  tmpNextNo = runSQL(codeToRun="SELECT COUNT(TypeID) + 1 FROM Usr_MRA_Overview WHERE TypeID = {origTypeID} AND EntityRef = '{entRef}' AND MatterNo = {matNo}".format(origTypeID=tmpTypeID, entRef=entRef, matNo=matNo)) 
  # now generate the final name to use
  finalName = "{typeName} ({nextNo})".format(typeName=tmpTypeName, nextNo=tmpNextNo)
  finalName = finalName.replace("'", "''")
  mra_Expiry = getSQLDate(selItem['MRA Expiry'])

  
  # generate SQL to copy high-level (Overview)
  insertOV_SQL = """INSERT INTO Usr_MRA_Overview (EntityRef, MatterNo, TypeID, ExpiryDate, LocalName, Score, RiskRating, ApprovedByHOD, DateAdded, copyOf)
                    SELECT '{entityRef}', {matterNo}, TypeID, DATEADD(WEEK, 4, '{expiryDate}'), '{localName}', Score, RiskRating, 'N', GETDATE(), {idItemToCopy} 
                    FROM Usr_MRA_Overview WHERE ID = {idItemToCopy}""".format(entityRef=entRef, matterNo=matNo, expiryDate=mra_Expiry, 
                                                                              localName=finalName, idItemToCopy=idItemToCopy) 
  #!^ 12/12/2025 - note as mentioned in 'Approve' function, this all takes far too long to run, so need to optimise our SQL calls where possible.
  #! here we are referring to 'MRA_Overview' and already refer to it above to get new Name, so could combine these into just this INSERT statement.
  #! I did start writing SQL for this (on SQL server, in my usual 'smippits' folder)
  ...
  # copy over the Questions - note: we copy EXACTLY AS-IS (including selected Answer, and 'Score')
  #! 12/12/2025 - I've been thinking about this and realised that we're locking matters into a specific template from the point of creation as we 
  #! never check to see if there's an updated template to use when copying questions across. 
  #! Also, this is where an issue lies with old 'scores' not appartently lining up with actual 'template Answer' scores... because we're just copying what the old one was!
  #! So, despite the fact I've worked on some SQL to lookup/get the latest template, because it's a bigger change than what I was expecting to do, 
  #! instead going to raise with Louis, and ask that he speak with Amy about to get their thought on what we do here.
  if int(rowID) > 0:
